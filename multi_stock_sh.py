...
def fix_hansen(A):
    trunc_A = [A[0][j] for j in range(674) if len(A[0][j]) >= 3800]

    stock_value = [[0 for j in range(len(trunc_A))] for i in range(3800)]
...
def main():
    data = scipy.io.loadmat('data_674_3800.mat')
    A = data['A']
    if (HANSEN):
    ...
    for fraction in xrange(FRACTION_NUMBER):
        ...
        algorithms += [StockAlgorithm(i, 0) for i in [0.001, 0.1, 1, 0, 100, 10000]]
